"use client";
import React, { ChangeEvent, useRef, useState } from "react";
import {
  StyledContainer,
  StyledImageContainer,
  StyledImageUploadText,
  StyledMessage,
} from "./ImageUpload.styles";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { createClient } from "@/utils/supabase/client";

export default function ImageUpload() {
  const supabase = createClient();
  const queryClient = useQueryClient();
  const [error, setError] = useState<string | null | boolean>(true);
  const fileInputRef = useRef<HTMLInputElement | null>(null);

  // 📥 Fetch the latest uploaded image
  const { data: latestImage } = useQuery({
    queryKey: ["latestImage"],
    queryFn: async () => {
      const { data, error } = await supabase.storage
        .from("user.images")
        .list("", {
          limit: 1,
          sortBy: { column: "created_at", order: "desc" },
        });

      if (error) throw new Error(error.message);
      if (!data?.length) return null;

      const { data: urlData } = supabase.storage
        .from("user.images")
        .getPublicUrl(data[0].name);

      return urlData.publicUrl;
    },
  });

  // 📤 Upload image and refetch latest image after success
  const { mutate, isPending } = useMutation({
    mutationFn: async (file: File) => {
      const {
        data: { session },
        error: sessionError,
      } = await supabase.auth.getSession();
      if (sessionError || !session?.user)
        throw new Error("User not authenticated.");

      const newFilePath = `user_${session.user.id}_${Date.now()}.png`;
      const { error: uploadError } = await supabase.storage
        .from("user.images")
        .upload(newFilePath, file);

      if (uploadError) throw new Error(uploadError.message);
      return newFilePath;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["latestImage"] });
      setError(null);
    },
    onError: (uploadError: Error) => setError(uploadError.message),
  });

  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return setError("No file selected.");
    if (!["image/jpeg", "image/png"].includes(file.type))
      return setError("Only JPEG and PNG allowed.");
    if (file.size > 2 * 1024 * 1024)
      return setError("File size should be less than 2MB.");
    mutate(file);
  };

  return (
    <StyledContainer>
      <StyledImageContainer
        onClick={() => fileInputRef.current?.click()}
        success={error ? "error" : "success"}
        style={{
          backgroundImage: latestImage ? `url(${latestImage})` : "none",
        }}
      >
        {/* <StyledImageUploaderImage
          src="images/icon-upload-image.svg"
          width={40}
          height={40}
          alt="image-placeholder"
        /> */}
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="40"
          height="40"
          fill="none"
          viewBox="0 0 40 40"
        >
          <path
            fill={latestImage ? "var(--white)" : "#633CFF"}
            d="M33.75 6.25H6.25a2.5 2.5 0 0 0-2.5 2.5v22.5a2.5 2.5 0 0 0 2.5 2.5h27.5a2.5 2.5 0 0 0 2.5-2.5V8.75a2.5 2.5 0 0 0-2.5-2.5Zm0 2.5v16.055l-4.073-4.072a2.5 2.5 0 0 0-3.536 0l-3.125 3.125-6.875-6.875a2.5 2.5 0 0 0-3.535 0L6.25 23.339V8.75h27.5ZM6.25 26.875l8.125-8.125 12.5 12.5H6.25v-4.375Zm27.5 4.375h-3.34l-5.624-5.625L27.91 22.5l5.839 5.84v2.91ZM22.5 15.625a1.875 1.875 0 1 1 3.75 0 1.875 1.875 0 0 1-3.75 0Z"
          />
        </svg>
        <StyledImageUploadText
          style={{ color: `${latestImage ? "var(--white)" : "var(--purple)"}` }}
        >
          {latestImage ? "Change Image" : "+ Upload Image"}
        </StyledImageUploadText>
        <input
          type="file"
          hidden
          ref={fileInputRef}
          onChange={handleFileChange}
        />
        {isPending && <StyledMessage>Uploading...</StyledMessage>}
      </StyledImageContainer>
      {isPending && <StyledMessage>Uploading...</StyledMessage>}
      {error && <StyledMessage>{error}</StyledMessage>}
    </StyledContainer>
  );
}
